---
description: Implement a plan. Default: all remaining increments in one run; pass "step" for only the next one. Does a hygiene pass, a qwe-critic loop, updates docs and the plan, runs tests, reports. Does NOT run the full reviewer (run /qwe-review or /qwe-review-branch on demand). Never commits or stages.
...
2. **Hygiene pass — do this explicitly, it's the step that gets skipped.** Before the critic pass, clean up your own diff:
...
4. **Test** the affected areas — run the project's test suite for what you touched. Fix failures before reporting.

5. **Update the plan** (per increment) — keep it lean:
   - Increment implemented, clean per the critic, and tested → REMOVE it from `## To do` and add ONE terse line under `## Done` (e.g. `- Add rate limiting — limiter middleware + config, removed ad-hoc checks`). Drop its sub-bullets.
...
6. **Report — terse, status first. No essays, no narration.**
   ...
   Nothing else — don't recount the critic loop or restate the diff. End with one line: review on your own terms, and run `/qwe-review` (uncommitted) or `/qwe-review-branch` (whole branch) for the full gate before a PR.

7. **Do NOT commit and do NOT stage.** Run no `git add`, no `git commit`, no branch creation. Leave every change as an unstaged working-tree modification — I will stage and commit it myself. (Use `/qwe-commit-message` when you want a message.)

8. In step mode, also propose the next unchecked increment as a checklist, without implementing it.
